...
> **satellite 004 (PLAN M0.5, 2026-09-17).** This folder was ported from 003
> revision 07's `src/programs/satl-term/`, nearly as-is, and is built by 004's
> `make` into `build/satl-term` beside `build/satl`. What changed: satl-term shows
> 004's title lines (`--version`, `--help` and on stderr at every start), a usage
> error exits 23 `command_line_not_understood`, a held tab names the machine code
> satl stopped on, and `--size` refuses a number no window can be given. Not
> ported: 003's window handover, which lived in satl, not here. Everything below
> is 003's record, and its milestones, sections and paths are 003's.

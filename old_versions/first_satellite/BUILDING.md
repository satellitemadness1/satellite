...
cd satellite/automatic_enterprise_build
c++ -std=c++20 -Wall -Wextra -O2 -o enterprise_satellite_builder build_and_install.cpp
./enterprise_satellite_builder
```

The driver lives in `automatic_enterprise_build/`, one directory below the top
of the tree, and it is built and run from there. It runs the four steps in the
order they have to happen in: uninstall, clean, build, install. `--help` prints
them and `--dry-run` prints them without running any.

If you would rather type the four commands, they are exactly these, **from the
top of the tree** rather than from the driver's folder:

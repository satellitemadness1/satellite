...
void Resolver::check_type(const Type &type, Span span)
{
    ...
    if (type.space == "container") {
        if (type.name == "list") {
            ...
            // NOT GENERIC, and it is the one container that cannot be. A result
            // holds sixteen fields of five different types at once, which is the
            // measured reason it is a type at all: there was no `map<K, V>` that
            // one could be bound to, and a `result<...>` would be inviting the
            // same argument back.
            if (!type.args.empty())
                fail(at, "satellite.container.result is not generic, so it "

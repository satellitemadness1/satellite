...
// The printer polls this while it is pacing. See the note in run() on why the
// console is the one place that can notice an interrupt in time.
...
void Console::abandon_pace()
{
    // Stored under pace_mutex_ rather than only in the atomic, so the printer
    // cannot read the old value, decide to wait, and go to sleep in between
    // this store and the notify below -- which is the classic lost wake-up and
    // would leave it asleep for the whole pause it was being told to skip.
    {
        std::lock_guard<std::mutex> guard(pace_mutex_);
        unpaced_.store(true, std::memory_order_relaxed);
    }
    pace_woken_.notify_all();
}

void Console::resume_pace()
{
    std::lock_guard<std::mutex> guard(pace_mutex_);
    unpaced_.store(false, std::memory_order_relaxed);
}

// Both reasons to stop waiting: told to, and Ctrl-C.
bool Console::pace_abandoned() const
{
    return unpaced_.load(std::memory_order_relaxed) || interrupt_requested();
}
    ...
    for (;;) {
        ...
        for (Piece &piece : batch) {
            ...
            if (pause > 0 && printed_any && owed.count() > 0 &&
                ...
                // wait_for IN SLICES, and not one sleep_for. Two different
                // things have to be able to cut a pause short, and they arrive
                // by different routes:
                //
                //   abandon_pace() NOTIFIES, so the predicate ends the wait at
                //   once. That is the explicit path.
                //
                //   A Ctrl-C does not notify anything. A signal handler may
                //   store into an atomic and nothing more -- it certainly may
                //   not take pace_mutex_ -- so the only way the flag can be
                //   seen is for this thread to look at it, and the only way to
                //   look at it during a 1500 ms pause is to wake up and look.
                //
                // THE PRINTER IS THE ONLY THREAD THAT CAN NOTICE IN TIME, which
                // is why console_output knows what an interrupt is at all. A
                // paced program queues its lines far faster than they print, so
                // by the time somebody presses Ctrl-C the walk has usually
                // finished and the run is already inside drain() -- there is no
                // other code left running to check anything. Measured: forty
                // lines at 300 ms, interrupted after 1.5 s, held the terminal
                // for the full 11.7 s before this loop existed.
                //
                // 20 ms slices bound the response to that key at 20 ms and cost
                // one extra wake per 20 ms of pause, on a printer that is
                // already flushing per line at any pace at all.
                constexpr auto SLICE = std::chrono::milliseconds(20);
                const auto deadline = std::chrono::steady_clock::now() + owed;

                std::unique_lock<std::mutex> paused(pace_mutex_);
                while (!pace_abandoned()) {
                    const auto now = std::chrono::steady_clock::now();
                    if (now >= deadline)
                        break;
                    pace_woken_.wait_for(
                        paused,
                        std::min<std::chrono::nanoseconds>(deadline - now,
                                                           SLICE),
                        [this] {
                            return unpaced_.load(std::memory_order_relaxed);
                        });
                }
            }

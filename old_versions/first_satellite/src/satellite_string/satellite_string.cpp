...
static_assert(PUNCT[punct_index(SAT_APOSTROPHE)] == '\'', "SAT_APOSTROPHE");
...
static const struct {
    ...
    {"\"", SAT_QUOTE},
    // \' is a REDUNDANT spelling of '. An apostrophe needs no escaping inside
    // a double-quoted string, and there are no single-quoted strings for it to
    // be escaping from — so a program that writes "DOESN\'T" is being
    // over-careful rather than wrong, and used to be punished for it by having
    // the backslash kept in the output. That was silent: no error, just a
    // stray backslash in every line, 501 of them in one real program.
    //
    ...
    // tell those apart from a mistake.
    {"'", SAT_APOSTROPHE},
};

...
// why this is in system_facts rather than beside the line editor.
...
// is what makes that true rather than assumed: an atomic that is not
// ALWAYS lock-free may take a lock, and a lock taken in a handler on a thread
// that already holds it is a deadlock with no stack to read.
std::atomic<unsigned> interrupts{0};
static_assert(std::atomic<unsigned>::is_always_lock_free,
              "the SIGINT handler stores into this; it must not take a lock");

// Whether the handler is already installed, so a second install_interrupt_
// handler() is free. Both entry points call it -- run_file_mode() and the
// repl -- and the repl's run_command() path reaches the first one indirectly.
...
void handler(int)
{
    ...
    if (previous >= 1) {
        ...
        //
        // The hook first, because a raw terminal outliving the
        // process is the one failure the user cannot recover from without
        // typing `reset` blind.
...
void install_interrupt_handler()
{
    ...
    // come back with EINTR so that satellite.console.input can tell a Ctrl-C
    // from a closed stdin, and so that a program waiting for a line can be
    // interrupted at all.
    action.sa_flags = 0;

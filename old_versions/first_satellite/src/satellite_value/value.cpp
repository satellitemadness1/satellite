...
struct SizeVisitor {
    ...
    // The fields, walked as a child, and the ranked list beside them. That list
    // is SHARED by every result of one resolution, and first_time() is what
    // bills it once rather than n times -- the same dedupe that lets §12's
    // cyclic object graph terminate, doing its other job. Without it
    // `found.size()` would grow with the SQUARE of the result count and report
    // bytes nothing occupies.
    size_t operator()(const ResultRef &result) const
    {
        if (!walk.first_time(result.get()))
            return 0;
        size_t total = walk.of_child(result->fields);
        if (result->ranked && walk.first_time(result->ranked.get())) {
            total += result->ranked->size() * VALUE_HANDLE_BYTES;
            for (const ValuePtr &other : *result->ranked)
                total += walk.of_child(other);
        }
        return total;
    }

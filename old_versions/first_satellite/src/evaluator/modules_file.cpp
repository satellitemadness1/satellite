    ...
    const std::vector<ValuePtr> &argv, Span span)
{
    ...
    // satellite.time.sleep(ms) -- the PROGRAM stops for a while.
    //
    // The deliberate opposite of satellite.console.display(100ms), and the two
    // are worth telling apart because they sound alike and are not alike. A
    // PACE meters the output and leaves the program running: the waiting is
    // done by the printer thread and the walk never learns it is being spaced
    // out (console.hpp, pace()). A SLEEP stops the walk itself -- nothing is
    // computed and no line is produced while it waits. One is a property of
    // the terminal, the other is a statement in the program.
    //
    // THE DRAIN IS WHY THIS IS NOT THREE LINES. Output is QUEUED, so a bare
    // sleep_for would pause a walk that is already thousands of lines ahead of
    // the terminal: the program would stop, the printer would carry on
    // emptying its backlog through the whole pause, and the gap would appear
    // somewhere the reader never asked for it -- or, on a long enough backlog,
    // not appear at all. read_input_line() has this exact problem with a
    // prompt and answers it this exact way, and drain() is already the
    // "everything queued is written AND flushed" barrier both of them need.
    // Draining first is the whole difference between "the program sleeps here"
    // and "the screen stops here", and only the second one is readable.
    //
    // MILLISECONDS AS AN ORDINARY NUMBER, not a duration literal. `100ms` is a
    // DurationLit the parser recognises for display() alone (expr.cpp), and a
    // module call takes ordinary arguments; a whole number of milliseconds is
    // what somebody asking for a pause reaches for anyway.
    if (full == "satellite.time.sleep") {
        if (argv.size() != 1) {
            fail(span, arity_message("satellite.time", "sleep", 1, argv.size()));
            return nullptr;
        }

        const Number *requested = std::get_if<Number>(argv[0].get());
        long long wait_ms = 0;
        if (!requested || !requested->to_integer(wait_ms)) {
            fail(span, "satellite.time.sleep wants a whole number of "
                       "milliseconds, got " + to_string(*argv[0]));
            return nullptr;
        }

        // Refused rather than floored to zero, which is the opposite of what
        // set_display_pace() does with a fraction, and deliberately so: a
        // fractional pace is a spacing nobody can observe, but a NEGATIVE sleep
        // is a caller that has computed its wait wrong, and silently not
        // waiting is how that bug survives to be found somewhere else.
        if (wait_ms < 0) {
            fail(span, "satellite.time.sleep cannot wait a negative length "
                       "of time");
            return nullptr;
        }
        ...
        // IN SLICES, and for the reason console.cpp gives at its own pause: a
        // Ctrl-C stores into an atomic and notifies nothing -- a signal handler
        // may not take a lock or wake a condition variable -- so the only way
        // this thread can see one during a long wait is to wake up and look.
        // A program that pauses between every step is a program somebody will
        // want to stop in the middle of a pause, and an uninterruptible
        // sleep_for would make Ctrl-C wait out the full length first.
        const auto slice = std::chrono::milliseconds(20);
        auto remaining = std::chrono::milliseconds(wait_ms);
        ...
        // Nil, like the display it stands beside. A sleep is done for its
        // effect on the clock and has nothing to hand back.

                                ...
                                const std::vector<ValuePtr> &argv, Span span)
{
    ...
    // satellite.system.threshold -- the search dial, and the one module
    // function whose arm is a FREE function rather than an Evaluator method.
    // search.hpp says why: the power is deliberately free of this class, and a
    // member for the dial would have made that claim false.
    {
        ValuePtr result;
        std::string error;
        if (const int handled = search_threshold_call(full, argv, result, error)) {
            if (handled < 0) {
                fail(span, error);
                return nullptr;
            }
...
bool Evaluator::read_input_line(std::string &line)
{
    ...
    // A Ctrl-C is not an end of input, and this is the one place in the
    // language where the two are easy to confuse. SIGINT is installed without
    // SA_RESTART (interrupt.hpp), so the read comes back with EINTR and
    // getline sets failbit -- exactly what a closed stdin does. eof() is what
    // separates them: a real end of input sets it and an interrupted read does
    // not. Without the clear(), every later read on this stream would fail too
    // and a program that caught the interrupt would find its input dead.
    if (!std::cin.eof() && interrupt_requested()) {
        std::cin.clear();
        line.clear();
    }
    return false;
...
ValuePtr Evaluator::console_input(const std::string &prompt, Span span)
{
    ...
    if (!read_input_line(line)) {
        ...
        //
        // A THIRD answer joins them: the read was interrupted. Saying "end of
        // input" about a Ctrl-C would be a false statement about the terminal,
        // and the message a person gets for pressing a key they pressed on
        // purpose should say what they did. exec() reports the interrupt again
        // at the next statement, which is harmless -- the first error is the
        // one that stops the walk and the one that gets read.
        if (interrupt_requested())
            fail(span, "interrupted");
        else

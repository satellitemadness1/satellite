...
bool value_equals(const Value &a, const Value &b)
{
    ...
    // A result is a handle to something with value semantics, so it needs the
    // arm the paragraph at the top warns about -- and it compares by its FIELDS,
    // which are the whole of what a result is.
    //
    // NOT by its alternatives, and that is not an omission. `rank` and
    // `candidates` are fields, so two results of different resolutions already
    // differ in the map whenever their standing differs; what is left out is the
    // twin an alternative is of the result in the returned list, and those two
    // ARE the same answer to the same question. Equality that separated them
    // would make `found[1] == found[0].alternatives()[0]` false about two values
    // that render identically, which is the bug this whole function exists to
    // stop being possible.
    if (const ResultBody *ra = as_result(a)) {
        const ResultBody *rb = as_result(b);
        if (!rb || !ra->fields || !rb->fields)
            return false;
        return value_equals(*ra->fields, *rb->fields);
    }

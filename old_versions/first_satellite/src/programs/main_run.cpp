...
void start_runtime()
{
    ...
    // Ctrl-C. Installed HERE and not at the top of main() because this is the
    // one function both ways of running a program pass through, and because
    // installing it is only meaningful once there is something to interrupt.
    // Idempotent, so the repl calling it once and every `run` at the prompt
    // reaching it again costs nothing. See system_facts/interrupt.hpp.
    satellite::install_interrupt_handler();
}

// Everything both run paths owe a program before it starts and after it ends.
//
// The CLEAR is the load-bearing half. An interrupt is a latch: it stays set
// until somebody puts it down, which is what lets the walk notice it a
// statement or two after the key was pressed. Not clearing it before a run
// would mean a Ctrl-C that stopped one program silently stopped the next one
// at its first statement, with a report naming a line the user had not looked
// at yet.
static void before_run(satellite::Console *console)
{
    satellite::clear_interrupt();

    // The prompt's Console outlives every program it runs, so a pace abandoned
    // by an interrupted run has to be handed back before the next one starts
    // -- otherwise one Ctrl-C would quietly disable
    // satellite.console.display(100ms) for the rest of the session.
    if (console)
        console->resume_pace();
}

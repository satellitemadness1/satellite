...
indistinguishable from a hung one. The prompt has a **line editor**: the arrow
keys move and recall, a history is kept across sessions in `~/.satl_history`
(`:history` says where, `$SATL_HISTORY=none` turns it off), and **Ctrl-C** means
the two things it should — at the prompt it cancels the line being typed and the
session lives, and during a run it stops the program at its next statement,
which exits 130.
...
Seventeen test binaries cover the above and all pass, including a ThreadSanitizer
...
same column. [§9 of the design docs](design/09-a-runtime-architecture.md)
...
make test            # seventeen test binaries, each PASS/FAIL on exit
make TSAN=0 test     # the other sixteen, for platforms without libtsan
...
[`DESIGN.md`](DESIGN.md) is the index; the design itself is twenty-one numbered
sections under [`design/`](design/), across thirty-one files, numbered so that

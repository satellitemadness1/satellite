...
reason they are not number literals in another base. §20 is still plan only. **§9 grew a third part**: the
prompt reads the arrow keys and keeps a history, and Ctrl-C means two different
things in the two places it can be pressed -- it cancels the line being typed,
and it stops a running program at its next statement. Neither worked at all
before: the up arrow was three literal characters and SIGINT had no handler, so
it took the session.
Last updated: 2026-08-24

This file is the index. The design itself is twenty-one numbered sections under
[`design/`](design/), across thirty-one files — one file per section, except where a
...
| §9 c | [The prompt, and Ctrl-C](design/09-c-the-prompt-and-ctrl-c.md) | §9.4–§9.10: the arrow keys, what Ctrl-C means in each of the two places, and the history |

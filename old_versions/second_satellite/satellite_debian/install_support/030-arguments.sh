...
#
# THE SAME OPTIONS AS THE ENTERPRISE INSTALLER, SPELLED THE SAME WAY. Nothing
# about parsing a command line is distribution-specific, and two installers for
# one language whose flags differed by a letter would be a trap rather than a
# convenience. Where a message names a package it names an apt one; everything
# else is that file's logic and that file's reasoning.
...
# ABSOLUTE. This path is written into a symlink in ~/.local/bin, and a relative
...
# link_bin is on by default and user_bin in 070-desktop.sh is $HOME/.local/bin,
# NOT derived from $root -- so without this a `--root /tmp/scratch` would write
# two symlinks into the real ~/.local/bin, pointing into a scratch directory
# that a later rm -rf makes dangling, and which an --uninstall without the same
# --root no longer recognises as its own. An explicit --link still wins, because
# someone who named both flags meant both.
...
# because 075-system.sh reports too and is sourced first. Telling someone their
# files were installed when the whole point of -n was that they were not is the
...
# though satellite owned it outright. A prefix layout does not make that mistake
# by construction: it writes bin/satl and share/icons/... , which are named
# files in directories it shares and does not own.
if [ "$layout" = root ]; then
    case $root in
...
install.sh: note -- --prefix /usr writes into the directory your package
            ...
            unknown to dpkg and apt and will not be replaced or removed by

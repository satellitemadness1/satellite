...
| **D24** | *(after T2, found by dark_mechanicum)* Every statement that merely READ a global took `satellite.library`'s exclusive hold, so threads reading constants queued: 8 threads 0.80 s against T1's 0.04 s; `dark_mechanicum_rising.satl` fed 37,650 items in 10 ticks against 4,079,650 on T1 (a `sleep(satellite.library.tick_seconds)` held the globals for a whole second). | `machine.cpp` `touch_globals` | run: `global_reads.satl` 0.80 s | T2 ✓ — the compiler marks statements that can write a global (`Compiled::statement_writes`); a read-only statement takes no hold; 0.03 s |
| **D25** | *(after T2, found by dark_mechanicum)* The access list let a thread that came straight back take a free entry past a waiter, so a watcher reading a busy object waited ~200 µs per read, and nothing promised it would ever get in. | `access_list.hpp` `acquire` | run: `busy_owner_reads.satl` 0.84 s | T2 ✓ — waiters are served first come, first served, after a short spin; 0.09 s |

| **D26** | *(D25's own fix, caught before commit)* The access list's atomics asked for `memory_order_acquire`; release() stores `owner = null` then reads `waiters` while a waiter does the reverse, and a store that sits in a core's store buffer let both miss each other: the entry was free, four threads slept on it, and nothing woke them. | `access_list.hpp` | run: `global_counter` 989 of 1,000 (11 timeouts); gdb on a hung run: owner null, waiters 4, all in `condition_variable::wait` | T2 ✓ — every atomic there is sequentially consistent; 0 hangs in 3,000 runs |

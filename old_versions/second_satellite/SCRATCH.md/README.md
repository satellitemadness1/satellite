...
| [PORTING.md](PORTING.md) | what `satellite_number` and `satellite_string` actually are in the first satellite, and what has to be decided before copying them. **Spent**: §4 and §5 landed at M6, the number half at M8 on 2026-08-31 with all four of its §6 questions answered in PLAN §6.1, and §3's own open question was struck at M10 — DESIGN §7.7 had already answered it and M10's console is what let the answer be demonstrated | **CONDITION MET — deletable now.** The live codes decode to real values (`satl` on a program printing `\threads` answers 24), PLAN §6.1 records what changed, and no section of this file is still asking anything |

...
// bignum.cpp. The BigInt half of that split came across UNCHANGED at M8 apart
// from the two things named below, because a magnitude was already unsigned and
// DESIGN §8.1's sign change is about the layer above this one.
...
// because compare() and add() live in different files.
//
// CHANGE ONE OF TWO: unsigned. v1 took a signed long long here because its
// significand carried the sign; the magnitude never does now, so the overflow
// check is the unsigned one and the table reaches one power further.
...
// CHANGE TWO OF TWO, and it is a DELETION rather than a rename. v1 asked
// whether a magnitude fit a LONG LONG -- the one signedness opinion this class
// ever had, and it existed only because v1's Number met its boxed form at a
// signed significand. The question this class can actually answer is whether
// the magnitude fits the small form's, and the conversion to a signed integer
// is Number::to_integer's, one layer up, where the sign lives.
//
// Three limbs is the size check and not the whole of it: 10^27 is over 2^64, so
// a three-limb magnitude may still not fit and the loop's own overflow guard is
// what decides. The final comparison against LLONG_MAX is what went.
...
unsigned long long BigInt::to_u64() const
{
    unsigned long long value = 0;
    for (size_t i = limbs_.size(); i-- > 0;)
        value = value * BASE + limbs_[i];

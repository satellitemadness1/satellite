// Print the VALUE, never N significant digits.
//
...
std::string Number::to_string() const
{
    ...
    // times in a printing loop. The sign is a character now rather than
    // something std::to_string reads off the significand.
    if (!big_ && exp_ == 0)
        return positive_ ? std::to_string(sig_) : "-" + std::to_string(sig_);
    ...
    // is_negative() and not !positive_, because zero is positive and this is
    // the one place printing the difference would be visible. The line above
    // has already returned for the only zero that reaches here.
    ...
    // The first satellite fixed the rule as "print the value" and set the
    // notation boundary at [1e-6, 1e21), because outside it "digits stop being
    // informative -- 1e308 in fixed notation is 309 characters". That reasoning
    // was written when a number was a double, where a 33-digit integer HAD no
    // 33 informative digits. It does now, so the boundary is restated in the
    // terms the reasoning was always really about:
    //

...
void Resolver::resolve_expr(const Expr &expr)
{
    ...
    // Every element is an ordinary expression in the enclosing scope, so a
    // literal may name locals: `{first_name, last_name}` resolves both.
    if (const ListLit *list = std::get_if<ListLit>(&expr)) {
    ...
    // The NAME is not a name in the program's sense — it is a keyword the
    // callee owns — so only the value is resolved.
    ...
    // NumberLit, StringLit, DurationLit and SatelliteLit hold no names.
}

...
StmtPtr Parser::parse_if()
{
    ...
    if (at_statement_keyword("else")) {
        ...
        // An EMPTY pair of parentheses is accepted and dropped:
        // satellite.statement.else() is the same statement as
        // satellite.statement.else.
        //
        // An else takes no condition, so the parens carry nothing — but every
        // other statement form in the language is written with them, and
        // reaching for them here by symmetry is the mistake a reader makes
        // once per program rather than once per career. Accepting the spelling
        // costs this much and refusing it costs an error that mentions only the
        // brace, which sends the reader looking at the block instead of at the
        // parens.
        //
        // Only an EMPTY pair. `else(x)` is still an error, because a condition
        // on an else is a misunderstanding rather than a spelling, and the
        // parse_expression inside parse_block's complaint is the right one to
        // get.

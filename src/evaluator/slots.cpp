                              ...
                              Span span)
{
    ...
    // TRUE and FALSE, LAST — after the library has been asked and has no
    // variable of that name. See the note in src/environment/names.cpp, which
    // is what routes them here: every scope the user owns gets asked first, so
    // this answers only for a word that names nothing at all.
    if (!v && (slot.name == "TRUE" || slot.name == "FALSE"))
        return make_value(slot.name == "TRUE");

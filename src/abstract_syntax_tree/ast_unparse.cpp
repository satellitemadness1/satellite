// unparse: a tree rendered back to source in canonical form. Every overload
// declared under "--- unparse ---" in ast.hpp lives here, along with the three
// static helpers only they use.
//
// Part of src/abstract_syntax_tree/, split out of a 369-line ast.cpp. What
// stayed behind is the tree's own machinery — spans, the SourceMap, the node
// constructors, and the operator table this file reads through precedence().
...
std::string unparse(const Expr &expr)
{
    ...
    // The spelling, unit and all. `100ms` is one token pair and one node, so
    // there is nothing to reassemble and nothing to get wrong.
    ...
    // `{}` for empty, and no trailing comma, so unparse emits the one spelling
    // the parser accepts and a round trip is a fixpoint.
    if (const ListLit *l = std::get_if<ListLit>(&expr)) {
        std::string out = "{";
        for (size_t i = 0; i < l->elements.size(); i++) {
            if (i)
                out += ", ";
            out += unparse(*l->elements[i]);
        }
        return out + "}";
    }

    if (const NamedArg *a = std::get_if<NamedArg>(&expr))
        return a->name + "=" + unparse(*a->value);

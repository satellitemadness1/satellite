...
| [src/system_facts/stack_facts.cpp](src/system_facts/stack_facts.cpp) | `RLIMIT_STACK` and this thread's own stack, ported whole. `RLIM_INFINITY` answers **unknown** and not unbounded. **M9's depth ceiling was derived from this until 2026-08-31 and there is no ceiling now** — DESIGN §7.5 was rewritten and the reader lost a consumer. The care it takes over the word is right either way, and `satl --limits` still prints what the machine says. |
...
which **M6** ports — so `decode()` emits `<threads>` and its five siblings until

...
| [src/satellite_words/words.def](src/satellite_words/words.def) | **The numbering, as data** — 254 nodes, 9 aliases, and since M14 one place row (`SAT_PLACE`, the third list: `input(prompt, target)`'s out parameter, one row by policy), a transcription of WORD_NUMBERS §2.2 and nothing else. A node's number is its **position** among its parent's children and is not a column. The one file exempt from PLAN §3's line ceiling. |

...
| [src/satellite_random/tiers.hpp](src/satellite_random/tiers.hpp) · [.cpp](src/satellite_random/tiers.cpp) | **M13.** The twelve shapes' arithmetic in two layers: `draw_*`, one answer each from whatever `Bits32` is handed in — the layer tests drive with a stub, so a hundred-run distribution check costs microseconds — and `tier_*`, the same draws behind DESIGN §11's window against one generator seeded once. The throwaway is whole answers of the shape being asked for. **M21 added a fourth tier and it is the one with no window**: `seed_stream`, `stream_is_seeded` and `draw_grid`, behind a `Source` the PROGRAM seeds rather than the kernel — held in a `unique_ptr` because a `Source` is neither copyable nor movable, so reseeding is a replacement. `draw_grid` is the language's only fractional draw: uniform over `(high − low) × 10^digits + 1` exactly-representable points, at the FINER of the dial and the bounds' own precision, so that *inclusive at both ends* stays true of a grid the bounds sit on. |
| [src/satellite_random/handlers.hpp](src/satellite_random/handlers.hpp) · [.cpp](src/satellite_random/handlers.cpp) | **M13, sixteen rows since M21.** The dice's join, on the console's model: twelve rows, `1 7 1`–`1 7 12`, with the three `.range` spellings free because M2 built them as aliases — **and M21's four at `1 7 13`–`1 7 16`, tagged M21 so `satl --words` says which milestone each row came from.** The seeded tier's door is the one that does NOT demand whole bounds (`plain_numbers`, `whole_numbers` without its second loop), asks `stream_is_seeded()` **before** it checks the bounds — a program that never seeded has a different problem from one that wrote a backwards range, and hearing about the range first sends it to the wrong line — and lets a step be fractional while keeping the exact-division rule that makes `max` reachable. Three rows are refusals by design — the zero-argument shapes hold S0901's author-text — and every argument check runs **before** any spin, random.hpp's lazy rule enforced at the door. |
| [src/satellite_random/handlers_internal.hpp](src/satellite_random/handlers_internal.hpp) · [seeded.cpp](src/satellite_random/seeded.cpp) | **M21.** The fourth tier, `satellite.random.seeded` `1 7 13`–`1 7 16`, and the three checks the two handler files share. **A file because the SUBJECT is separate and not because of an arithmetic** — PLAN §3's line rule says *"split by subject when you split at all, because a seam chosen to satisfy an arithmetic is a seam in the wrong place"*, and warns in the same breath about *"headers named `eval_internal.hpp` existing to hold what an anonymous namespace used to"*, which is exactly what this one is. The subject: `seeded` has **no throwaway window**, holds a stream the PROGRAM seeds rather than the kernel, and is the module's only door that accepts a fractional bound — where the other three differ from each other in nothing but how long they take. `satellite_scalars/hex_methods.cpp` is the precedent, split from `bits_methods.cpp` on the day the second radix arrived. `install_handlers()` stays the module's one door and calls this file's half. |
| [src/satellite_thread/thread_handle.hpp](src/satellite_thread/thread_handle.hpp) · [.cpp](src/satellite_thread/thread_handle.cpp) | **M23.** The two bodies behind `satellite_value/value.hpp`'s tenth and eleventh arms. `Deferred` is `satellite.variable.capsule` `1 6 16` — a capsule index and the argument VALUES, computed on the calling thread and frozen, which is what makes it a value rather than a closure: no environment is captured and no slot is reachable. `ThreadHandle` is `satellite.variable.thread` `1 6 13`, and it is the SECOND arm ever to come off the `const` in the variant, after the file, for the file's reason: two names for one thread are one thread. **The OS thread is a fresh `std::thread` and deliberately not the M6 pool's** — the header carries the argument PLAN §4.5.1 now agrees with. `close_all()` is the rule every entry point that runs a program obeys: raise `stop`, wake anything blocked, join, and hand back the refusals of threads nobody waited for. |
| [src/satellite_thread/handlers.hpp](src/satellite_thread/handlers.hpp) · [.cpp](src/satellite_thread/handlers.cpp) | **M23.** Three rows for six numbered paths, and the header says why the other three are right to have none: `1 23` and `1 6 13` are a namespace and a type that `satellite_help/built.cpp` derives from the children below them, and `1 6 13 0` / `1 23 0` are bare shapes. `new` binds no receiver; `start()` and `join()` bind one and **neither mutates** — a thread is a reference type, so `start()` changes what the handle points at rather than which handle the slot holds, which is the first time DESIGN §6.4's last line has been load-bearing in the *negative*. |
| [src/satellite_spaceship/shape.hpp](src/satellite_spaceship/shape.hpp) · [.cpp](src/satellite_spaceship/shape.cpp) | **M25's include half, 2026-09-13.** What a `satellite.include(...)` names, read once over the arena: nothing, the runtime, one of the four spellings of a spaceship (`ship`, `ship(args)`, `ship.satl`, `ship.satl(args)`), or not a name at all. The loader, the resolver and the compiler all ask this one function, so a fifth spelling cannot be half-accepted. |
| [src/programs/spaceships.hpp](src/programs/spaceships.hpp) · [.cpp](src/programs/spaceships.cpp) | **M25.** A program of more than one file: every spaceship found beside the file that includes it, parsed under its own `satellite.library.<ship>` node, then every file resolved together (`resolve::resolve_run`) and compiled into one program (`eval::compile_run`). A program that includes nothing never reaches it. S1601–S1606 are its refusals. |
| [src/name_resolver/spaceships.cpp](src/name_resolver/spaceships.cpp) | **M25.** Names that live in another file: `ship.setup`, `ship.box` in type position, and which of a spaceship's declarations a name is. A spaceship is a namespace and not a scope — every road starts from a name the including file wrote. |
| [src/evaluator/compile_includes.cpp](src/evaluator/compile_includes.cpp) · [operations_include.cpp](src/evaluator/operations_include.cpp) | **M25.** An include compiles to one op, `op_include`, carrying the file and the argument ops. When it runs it matches the values to every `satellite.capsule.launch` they fit, refuses S0734 if none, runs the file's top-level includes the first time (holding the file's loading access once there can be threads), and enters each fitting launch in file order with the include as its call site. |
| [src/evaluator/globals.hpp](src/evaluator/globals.hpp) | **M23.** `satellite.library`, as the one thing two walks share — DESIGN §7.2's *"permanent identity, CROSS-THREAD SHARING, lock-free reads"*, which asked for this four years of milestones early. Lock-free reads are what it asks for and not what this is: a `Value` is a 40-byte variant and no machine this tree targets reads one atomically, so there is a mutex and **what is lock-free is deciding whether to take it**. A program with no second walk pays one relaxed load, and 3,000,000 global reads against 3,000,000 local reads are indistinguishable on this machine. A handle and not a singleton, for `dispatch.hpp`'s opposite reason: the handler table is a property of the BUILD, these are a property of a RUN, and M22's prompt runs many programs in one process. |
...
| [src/satellite_value/value.hpp](src/satellite_value/value.hpp) · [.cpp](src/satellite_value/value.cpp) | **M9.** DESIGN §8's value, at §8.2's forty bytes with a `static_assert` on it — **thirteen arms since M20**, append-only, and nothing wider than a `Number` goes inline, which is why §8.6's float cannot and why `Flo`, `Lst`, `Map`, `Fil`, `Bin`, `Hex` and `Arg` all arrive behind sixteen-byte handles. The assert has not moved once across NINE appends, which is the whole point of measuring each one against `Str` rather than against the budget — and M9's own entry named `Arg` as a future append four milestones before it arrived, with the instruction to re-run the assert when it did. Plus the three things every arm shares: what a value is CALLED in a sentence, whether it is true (only a bool is — there is no truthiness), and whether two are equal. |
| [src/satellite_value/value_arguments.hpp](src/satellite_value/value_arguments.hpp) | **M20.** The body behind the ninth arm: the command line, each word with the name it answers to, and **the one machine fact that could not be deferred** — `getcwd()` at startup, because `satellite.directory.change` `1 18 1` can move the process afterwards and the arguments object is what the program WOKE UP to. The other thirty-two are lazy and live in `system_facts/`. Built, then frozen, which is `MapBody`'s rule for `MapBody`'s reason. |
| [src/satellite_value/render.hpp](src/satellite_value/render.hpp) · [.cpp](src/satellite_value/render.cpp) | **M9.** A value as characters, and **the one place in the tree that answers DESIGN §5's six live codes** — PLAN §6.1's "six calls", made here and not in the alphabet. A string with no live code in it never asks the machine anything. **Since M20 it has one arm it does not implement**: the arguments object prints all of itself (§7.7) and `src/satellite_arguments/render.cpp` is that walk, so every binary linking this file links that module too. |
...
| [src/satellite_scalars/float_methods.cpp](src/satellite_scalars/float_methods.cpp) | **M21, and it is ONE ROW.** `satellite.variable.float.to_string()` `1 6 10 1` — the type's first method, because `1 6 10` was a leaf in `words.def` from M2 until M21. The float landed at M15 with four operations, a rounding rule and a dial and nothing a program could *ask* it, which is defensible: arithmetic and `display` are most of what a float is for. **What found the edge of it was QUAD's `Sky::save`** — `Float::to_string()` had existed in C++ since M15 with one caller, `satellite_value/render.cpp`, so a value a program could SEE was one it could not put in a string (S0711 on `+`) or write to a file (S0713 on `write_line`), while `Sky::load` already worked because `to_number()` answers. **A round trip available in one direction only** is what made it a number rather than a note. Renders exactly what `display` renders — a second spelling would let a program's file and its screen disagree about one value, DESIGN §4.6's drift. Carries `float_at`, the receiver check `methods_internal.hpp`'s other four have had since M11. |
...
| [src/satellite_system/group_map.hpp](src/satellite_system/group_map.hpp) · [.cpp](src/satellite_system/group_map.cpp) | **M20.** A group word displaying its children — the shape the author chose for the arguments object's groups on 2026-09-11 and chose again for `satellite.system.memory()` `1 22 4 0` the same day. **The children come out of the registry and the unit shapes collapse into them**: `free()` `1 22 4 6` and `free(unit)` `1 22 4 10` are two rows and one WORD, so the map is keyed by the word and answered by the arity-0 shape — the default unit by construction rather than by a choice made here. A child with no no-argument answer is **left out** rather than filled with `nothing`, which today is `this` `1 22 4 5`. A file of its own because `satellite.system` has four such group words and `memory_methods.cpp` was at 352 lines with this inside it. |
| [src/satellite_arguments/arguments.hpp](src/satellite_arguments/arguments.hpp) · [.cpp](src/satellite_arguments/arguments.cpp) | **M20.** `satellite.library.main.arguments` — DESIGN §7.7's object, and the module's own half of it: the command line, the directory it was started in, and the one `Value` a run ever builds of it. **Process-wide, and the handlers take no receiver** — there is one command line and one machine per run, so a fact under `arguments` compiles to the same `op_dispatch` a module constant does and answers from here. The VALUE exists for what needs a value: `display(arguments)`, `arguments[i]`, and handing the object to a capsule. |
| [src/satellite_arguments/rows.hpp](src/satellite_arguments/rows.hpp) · [.cpp](src/satellite_arguments/rows.cpp) | **M20.** Which registry number is which fact — thirty-two rows, each a pointer to a field of `facts::MachineAnswers`, plus the two the OBJECT knows rather than the machine (`session.directory` and `count`). **Split out of handlers.cpp for a LINK reason and not a size one**: the value renderer needs the answers, and if they lived beside the handlers then every binary linking that renderer would drag the evaluator's dispatch table in with them. Nothing here knows what a Machine is. |
| [src/satellite_arguments/render.cpp](src/satellite_arguments/render.cpp) | **M20.** The object as text — §7.7's *"displaying it bare prints all of it"*. Two columns, one entry per line, v1's printer ported; it is the one rendering in the language that is not a single line. The names are the paths RELATIVE to `arguments`, dotted, so what a reader sees printed is what they would type. |
| [src/satellite_arguments/handlers.cpp](src/satellite_arguments/handlers.cpp) | **M20.** The thirty-six rows the evaluator dispatches. **One template body and thirty-two function pointers**, because a handler is handed no path to look itself up by. Six **group** words display their children as a map — the author's call, 2026-09-11 — and `interpreter` is the exception PLAN's shape table had already settled, answering the path to the binary. `library_path` and `library_path_source` **refuse with the reason and name M25**, which is where `satellite.include` of another file lands and the first milestone at which a library path means anything. |
...
| [src/system_facts/arguments_facts.hpp](src/system_facts/arguments_facts.hpp) · [.cpp](src/system_facts/arguments_facts.cpp) | **M20.** The thirty-two facts the arguments object answers about this machine and this build, ported from v1's `arguments_facts.cpp` and its `arguments_for()`. **BUILT ONCE, ON FIRST ASK** — a function-local static, so a program that never reads a fact pays for none of the dozen syscalls and the `/etc/os-release` parse. That is the one place in this directory that caches, and the exception is argued rather than assumed: facts.hpp caches nothing because *"the question is what this program is using NOW"*, and every fact here is fixed for the life of the process. The third object the Makefile stamps with `$(VERSION_DEFS)`, because `build.*` describes **the binary that is running**. |

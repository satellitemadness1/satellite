...
#
# Taken from ../satellite_enterprise/install_support/020-saying-things.sh
# unchanged in behaviour, because they were right there and nothing about them
# is distribution-specific. Only usage() differs, and only where it names a
# package or a path.
...
This is the Debian family installer -- Debian 13 and Ubuntu 24.04 or newer.
It builds satellite_debian/, which is an out-of-tree build: the four binaries
are made in build/ beside this script and the tree above is only read.
...
satl.haswell is not a fourth program, it is satl compiled a second time.
  ...
  --desktop      also symlink the icons, the .satl file type and the satl-term
                 ...
                 three indexes covering every application on the machine.
  ...
  --static       link the C++ runtime into the programs instead of loading it
                 ...
                 which on this family is almost always: libstdc++.a ships in
                 g++ and libc.a in libc6-dev, and build-essential depends on
                 both. It costs about a megabyte a binary and buys one that
                 runs with an empty environment and no compiler present.
                 --no-static turns it off. satl-term is never fully static --
                 gtk and vte load their own modules -- but its C++ runtime is
                 linked in like the rest.
...
what to install first:
  ...
  sudo apt install gtk-update-icon-cache desktop-file-utils shared-mime-info \\
                   hicolor-icon-theme                       (for --desktop)

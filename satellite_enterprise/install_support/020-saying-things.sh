...
Installs the interpreter, the .satl file type and the artwork into
    $root
...
this CPU which of them it can execute, and installs that one.

  --link         also symlink ~/.local/bin/satl at the installed interpreter,
                 so that typing \`satl\` finds it. Off by default because that
                 path may already hold another satl -- this machine's does --
                 and this script refuses to overwrite anything it does not
                 own. It never edits a shell startup file either way.
  --desktop      also symlink the icons and the .satl file type into
                 ~/.local/share, which is where a desktop actually looks.
                 Without it they are installed under the root above, which no
                 desktop reads, so .satl files keep whatever icon they already
                 had. The interpreter is unaffected either way.

...
struct Capsule : Obj {
    ...
    // The canonical library location -- where variables declared INSIDE this
    // capsule live, as satellite.library.<location>.<var>.  Not where the
    // capsule itself is registered: that is the global location, see
    // register_capsules() below.
    std::string        location;

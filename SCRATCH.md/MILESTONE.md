...
| the **thread pool**, sized from `THREAD_COUNT`, shared by the console printer, parse-time interning and M12 | PLAN §4.5.1, **§4.5.1.2** | no milestone. M12 is the closest and says nothing about a pool. **It stopped being "lazy" on 2026-08-28**: the author ruled it is built at startup, always, because most satellite code is expected to call `parallel_for` — which raises the stakes on this row, since the pool now runs before anything else does |
...
| ~~the **crossover measurement**~~ | PLAN §4.5.1 | **MEASURED 2026-08-28 and struck from this file.** ~2,650 lines before 24 fresh threads beat one — **and the same ~2,650 for a pool built to do the job**, since building one costs ~590 µs. **~170** applies only to a pool something else already warmed, so a one-shot parse should not thread at all. It could not be taken before M2, because the walk it measures did not exist. The answer is in PLAN §4.5.1 beside the decision it justifies |
...
| ~~**`satellite.variable.binary` and `.hex`**~~ | DESIGN §8.5 | **STRUCK 2026-08-28 — PLAN M3 owns them and says so.** This row said "M3 *may* already own them without saying so"; §8's M3 entry has named them explicitly since 2026-08-27, together with `hexadecimal` as the one alias the lexer's spelling table has to know. The row outlived the gap it recorded |

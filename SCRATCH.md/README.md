...
| [NO_LIMITS.md](NO_LIMITS.md) | **The author's rule that the language has no limits, and the four walkers that break it today.** What the 8 MiB, the 20,000 and the 2,000 each actually are — only one is a limit and it was added at M7 — every recursive walker in the tree, the measured depth at which each command dies, the plan file by file, and what changes in DESIGN §7.5, PLAN §2.5 and M9 | the walkers keep their own stacks, nothing segfaults at any depth, and the three permanent documents carry it |

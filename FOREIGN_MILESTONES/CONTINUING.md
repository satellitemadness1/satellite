...
| [PYTHON/](PYTHON/) | 323 features | 323 | **complete** 2026-09-12 — every row has a page, every link resolves; 64 rows are **open** |
...
2. **JAVA/ and RUST/ pages** — the user's stated order after C++. **PYTHON/ is
   done** (2026-09-12): 323 rows, one page each, generated by
   [tools/gen_python.py](tools/gen_python.py). Its open rows are listed and
   grouped by shared decision at the foot of [PYTHON/README.md](PYTHON/README.md).
   The flagged rows still to page: Java's garbage collection vs refcounting
...
4. **Corrections found 2026-09-12 that belong to the interpreter session**, not
   ...
   **Found writing PYTHON/, 2026-09-12**, each checked with a probe program:
   - **`q = 7 // 2` runs and sets `q` to 7.** `//` opens a comment, so Python's
     floor division is silently deleted and `--check` has nothing to say. A
     diagnostic for `//` directly after an operand is worth having
     ([PYTHON/M90](PYTHON/M90.md)).
   - **A `satellite.variable.number` accepted `x = "abc"`** and displayed `abc`,
     with no refusal at check time or run time ([PYTHON/M33](PYTHON/M33.md)).
   - **`max_depth` is `unset` in `satl --limits`**, and a capsule recursing
     without end ran past 20 seconds with no output ([PYTHON/M147](PYTHON/M147.md)).
   - `2.power(3)` displays `8.0`, while `10 / 4` displays `2.5`
     ([PYTHON/M62](PYTHON/M62.md)).
   - `1 < x < 3` refuses with `S0712: bool has no order`, which is true of the
     parse and unhelpful to the reader ([PYTHON/M96](PYTHON/M96.md)).
   - `1e3` does not lex as a literal, yet `"1e3".to_number()` is `1000`
     ([PYTHON/M77](PYTHON/M77.md)).
   - `remove_first()` and `remove_last()` run on a list but are not listed by
     `satl --words`.
...
**[tools/gen_python.py](tools/gen_python.py) is the better model to copy for
JAVA/.** It improves on the CXX23 generator in three ways:

- **Rows refer to each other by name, not number**: `@{duck typing}@` in a
  page's text becomes `[M20](M20.md)` after numbering (a key is a feature name, a
  unique prefix of one, or `=name` for an exact match), and the run stops if a
  key matches no row or more than one. The first draft typed numbers before the
  numbering existed, and they could not have been right.
- Code is written as raw triple-quoted strings, so a backslash in Java or satellite
  source needs no escaping. A `"""` inside the text needs `r'''` around it.
- Rows 1–20 are regenerated as pages too, so every index link resolves on the
  first run. It writes the index to a path given on the command line, and
  `PYTHON/README.md` is assembled from that plus a hand-written header and footer.

**Check `CXX23/M<n>` references against the page title before citing them.**
Rows 1–56 mostly have no page (link `../CXX23/README.md` for those), and three of
the CXX23 numbers first cited from memory in PYTHON/ were a neighbouring row.

**Probe runner.** A shell loop that wraps each snippet in `satellite.include` and
`satellite.main`, runs it under `timeout 10`, and prints the first lines of
output settled about 130 questions for PYTHON/ in three runs. A snippet can
declare file-scope capsules above a `@MAIN` marker. Rebuild it in the scratchpad;

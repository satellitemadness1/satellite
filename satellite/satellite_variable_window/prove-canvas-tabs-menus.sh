...
# CANVAS, CLICK A TAB, WALK A MENU INTO A MENU, on a compositor of its own.
# GTK_AND_NO_DEPENDENCIES.md GTK-15, GTK-16 and GTK-12's leftovers (2026-09-22).
#
...
#           IBM Plex Mono 18) BEFORE it is on the screen and saved to a PNG; a
#           real pointer click on it runs its .clicked capsule, which is handed
#           the canvas and its window, draws MORE on it, saves a second PNG and
...
[ "$canvas_size" = "400 300 400 300 " ] &&
[ "$tabs_status" = "exit 0" ] && [ "$tabs_order" = "$want_tabs" ] &&
[ "$menus_status" = "exit 0" ] && [ "$menus_picked" = "1" ] || {
    echo "prove-canvas-tabs-menus.sh: FAILED"; exit 1; }
echo "prove-canvas-tabs-menus.sh: a canvas was drawn on, a tab was clicked and a menu was walked -- and satellite ran"

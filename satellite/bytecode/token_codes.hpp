...
inline constexpr Code arrow_token = 0x070A;  // -> QUESTION: no return type is written; satellite.returns is gone
...
inline constexpr Code transparency_token = 0x0B4F;  // transparency  [METHOD] color.transparency(50) sets how see-through a colour is, 0 to 99;
inline constexpr Code numerator_token = 0x0B50;  // numerator  [METHOD] fraction.numerator -- the number above the line of a/b
inline constexpr Code denominator_token = 0x0B51;  // denominator  [METHOD] fraction.denominator -- the number below the line of a/b
inline constexpr Code sum_token = 0x0B52;  // sum  [METHOD] list.sum -- every item added with +, and 0 for a list of nothing (003's li
inline constexpr Code max_token = 0x0B53;  // max  [METHOD] list.max -- the largest item by what it is worth, one pass and no sort (00
...
inline constexpr Code start_token = 0x0B58;  // start  [METHOD] thread.start() -- the capsule begins on a thread of its own, and start a
inline constexpr Code stop_token = 0x0B59;  // stop  [METHOD] thread.stop() -- asks the thread to stop at its next statement; it is nev
inline constexpr Code wait_method_token = 0x0B5A;  // wait  [METHOD] thread.wait() -- a second name for thread.join() (the author, 2026-09-23)
inline constexpr Code lock_token = 0x0B5B;  // lock  [METHOD] object.lock() -- turns the object's lock ON; it is taken only while somet
inline constexpr Code unlock_token = 0x0B5C;  // unlock  [METHOD] object.unlock() -- turns the object's lock off again: nothing is locked
inline constexpr Code upper_token = 0x0B5D;  // upper/uppercase/up  [METHOD] string.upper() -- the text in capital letters, every langua
inline constexpr Code lower_token = 0x0B5E;  // lower/lowercase  [METHOD] string.lower() -- the text in small letters (the author, 2026-
inline constexpr Code center_token = 0x0B5F;  // center/centre  [METHOD] satellite.console.display(text).center() -- the line in the midd
inline constexpr Code starts_with_token = 0x0B60;  // starts_with  [METHOD] string.starts_with(x) -- whether the string begins with x; x of ""
inline constexpr Code ends_with_token = 0x0B61;  // ends_with  [METHOD] string.ends_with(x) -- whether the string finishes with x (M16, word
inline constexpr Code substring_token = 0x0B62;  // substring  [METHOD] string.substring(start, end) -- the characters from start to end, bo
inline constexpr Code split_token = 0x0B63;  // split  [METHOD] string.split(separator) -- the pieces between separators as a list of st
inline constexpr Code trim_token = 0x0B64;  // trim  [METHOD] string.trim -- the text without space, tab, carriage return or newline at
inline constexpr Code at_token = 0x0B65;  // at  [METHOD] string.at(n) -- character n, counting from 1, as a string of one character;
inline constexpr Code resolved_token = 0x0B66;  // resolved  [METHOD] string.resolved -- the string with its live escapes resolved; 004 has
...
inline constexpr const char *method_name_of(Code code)
{
    ...
    if (code == transparency_token) return "transparency";
    if (code == numerator_token) return "numerator";
    if (code == denominator_token) return "denominator";
    if (code == sum_token) return "sum";
    if (code == max_token) return "max";
    ...
    if (code == start_token) return "start";
    if (code == stop_token) return "stop";
    if (code == wait_method_token) return "wait";
    if (code == lock_token) return "lock";
    if (code == unlock_token) return "unlock";
    if (code == upper_token) return "upper";
    if (code == lower_token) return "lower";
    if (code == center_token) return "center";
    if (code == starts_with_token) return "starts_with";
    if (code == ends_with_token) return "ends_with";
    if (code == substring_token) return "substring";
    if (code == split_token) return "split";
    if (code == trim_token) return "trim";
    if (code == at_token) return "at";
    if (code == resolved_token) return "resolved";
    return "";
}
...
inline constexpr Code method_code_of(std::string_view spelling)
{
    ...
    if (spelling == "transparency") return transparency_token;
    if (spelling == "numerator") return numerator_token;
    if (spelling == "denominator") return denominator_token;
    if (spelling == "sum") return sum_token;
    if (spelling == "max") return max_token;
    ...
    if (spelling == "start") return start_token;
    if (spelling == "stop") return stop_token;
    if (spelling == "wait") return wait_method_token;
    if (spelling == "lock") return lock_token;
    if (spelling == "unlock") return unlock_token;
    if (spelling == "upper") return upper_token;
    if (spelling == "uppercase") return upper_token;
    if (spelling == "up") return upper_token;
    if (spelling == "lower") return lower_token;
    if (spelling == "lowercase") return lower_token;
    if (spelling == "center") return center_token;
    if (spelling == "centre") return center_token;
    if (spelling == "starts_with") return starts_with_token;
    if (spelling == "ends_with") return ends_with_token;
    if (spelling == "substring") return substring_token;
    if (spelling == "split") return split_token;
    if (spelling == "trim") return trim_token;
    if (spelling == "at") return at_token;
    if (spelling == "resolved") return resolved_token;
    return 0;
}

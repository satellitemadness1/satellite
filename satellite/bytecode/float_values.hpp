#pragma once
// satellite/bytecode/float_values.hpp -- EVERYTHING THE CHECKER AND THE WALKER ASK
// ABOUT A FLOAT, `satellite.variable.float` (1 6 10). The value itself is
// satellite_object/object_float.hpp's.
//
...
// THE CHECKER, BEFORE ANYTHING RUNS: the value given to a name declared satellite.variable.float,
...
signed long long int float_is_written_right(const std::vector<std::bitset<16>> &row, std::size_t at,
                                            const std::unordered_map<std::string, token::Code> &declared,
                                            std::string &why);

// THE CHECKER: the first method written on a name declared satellite.variable.float. `spelling`
...
// change `value` (a number given to a float name becomes a float).
signed long long int float_on_store(token::Code holds, Value &value, std::string &why);
...
Value float_method(token::Code method, const Value &receiver, Value *slot, const std::vector<Value> &arguments,

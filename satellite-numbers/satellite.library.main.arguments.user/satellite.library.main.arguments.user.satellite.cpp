// satellite.library.main.arguments.user  `1 14 1 1 15` -- the login name this interpreter is running as.
//
// AN ALIAS OF `arguments.username`, on the author's word (2026-09-18):
...
// machine_facts.hpp's answer_username(), so the two cannot answer differently,
...
extern "C" signed long long int satellite_number_describe(satellite004::LibraryRow *row)
{
    ...
    row->name = "satellite.library.main.arguments.user";
    row->numbers[0] = 1;
    row->numbers[1] = 14;
    row->numbers[2] = 1;
    row->numbers[3] = 1;

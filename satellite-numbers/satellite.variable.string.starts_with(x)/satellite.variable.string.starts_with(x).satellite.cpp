// satellite-004/satellite-numbers/satellite.variable.string.starts_with(x)/satellite.variable.string.starts_with(x).satellite.cpp
//
// satellite.variable.string.starts_with(x)   1 6 1 6
// True when the string begins with the text.
//
// A library with no main: it exports satellite_number_describe (number_row.hpp),
// which the interpreter calls once at start-up. Built as 1.6.1.6.so by
// build_libraries.py. Behaviour ported from 003 06's
// src/satellite_scalars/string_methods.cpp; written 2026-09-14.

#include "../number_row.hpp"
#include "../../machine_codes.hpp"

namespace {

using namespace satellite004;

signed long long int method(satellite_string &self, const StringArguments &arguments, StringAnswer &answer)
{
    if (arguments.strings.size() < 1)
        return error;
    const std::u32string &head = arguments.strings[0].text;
    answer.kind = StringAnswer::Kind::flag;
    answer.flag = self.text.size() >= head.size() && self.text.compare(0, head.size(), head) == 0;
    return success;
}

} // namespace

extern "C" signed long long int satellite_number_describe(satellite004::LibraryRow *row)
{
    row->name = "satellite.variable.string.starts_with(x)";
    row->numbers[0] = 1;
    row->numbers[1] = 6;
    row->numbers[2] = 1;
    row->numbers[3] = 6;
    row->depth = 4;
    row->scenarios.string_method = method;
    return satellite004::success;
}

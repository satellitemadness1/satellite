...
// first two being distinguishable — eval/slots.cpp reports "is read before its
...
//
// The ceiling is the one the language ALREADY promises. eval/helpers.cpp carries
// DEFAULT_MAX_DEPTH = 2000 with a ceiling of 3000, reachable through
// satellite.library.system.max_depth, so the bound here is chosen to honour that
// rather than to fit a cache: a program that recurses to the documented limit
// must not run out of registers first.
//
